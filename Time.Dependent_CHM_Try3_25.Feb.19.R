#TO FIGURE OUT THE ISSUE, DOING "POINT MUTATIONS" ON EVERY CHANGE BETWEEN THE 
...
age     <- 20                                #NOT AGE # age at baseline 
...
### Start
a.P_notrt["Get_GT", "Healthy",] <- 1

### From Healthy
a.P_notrt["Healthy", "Healthy",]  <- p.Healthy_Healthy
a.P_notrt["Healthy", "Hindered1",] <- p.Healthy_Hindered1
a.P_notrt["Healthy", "Hindered2",] <- p.Healthy_Hindered2
a.P_notrt["Healthy", "Blind2",]  <- p.Healthy_Blind2
a.P_notrt["Healthy", "Death", ]  <- pDead
...
### Start 
a.P_trt["Get_GT", "Healthy",] <- p.GT.fail
a.P_trt["Get_GT", "GT_Healthy",] <- p.GT.success

### From GT_Success
a.P_trt["GT_Healthy", "GT_Healthy",] <- p.GT.GTHealthy_GTHealthy
a.P_trt["GT_Healthy", "GT_Hindered1",] <- p.GT.GTHealthy_GTHindered1
a.P_trt["GT_Healthy", "Blind2",] <- p.GT.GTHealthy_Blind2
a.P_trt["GT_Healthy", "Death",] <- pDead
...
#tc.d_trt
#SOC incremental 
GT_discounted_cycle_utility_15 <- t(v.dwe*t(v.tu_trt))
GT_discounted_cycle_cost_15 <- t(v.dwc*t(v.tc_trt))
SOC_discounted_cycle_utility_15 <- t(v.dwe*t(v.tu_no_trt))
SOC_discounted_cycle_cost_15 <- t(v.dwc*t(v.tc_no_trt))

write.csv(GT_discounted_cycle_utility_15, file = "GT_discounted_cycle_utility_15")
write.csv(GT_discounted_cycle_cost_15, file = "GT_discounted_cycle_cost_15")
write.csv(SOC_discounted_cycle_utility_15, file = "SOC_discounted_cycle_utility_15")
write.csv(SOC_discounted_cycle_cost_15, file = "SOC_discounted_cycle_cost_15")
